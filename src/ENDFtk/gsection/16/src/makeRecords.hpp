...
makeRecords( double temp,
    ...
    for ( size_t g_i = 0; g_i < ngroups; ++g_i ) {
        ...
        std::cout << "calc ig2lo :" << ig2lo << std::endl;
        for ( size_t g_o = 0; g_o < max_gg; ++g_o ) {
            ...
            for ( size_t l = 0; l < nmoments; ++l ) {
                for ( size_t z = 0; z < ndilutions; ++z ) {
                    ...
                    if ( g_o >= ( ig2lo ) && matrix[l][z][g_i][g_o] != 0.0 || ( g_i == (ngroups - 1) && g_o == 0 ) ) {
                        std:: cout << " matrix: " << matrix[l][z][g_i][g_o] << std::endl;
        ...
        if ( g_i == ngroups - 1 ) {
            for ( size_t l = 0; l < nmoments; ++ l ) {
                for ( size_t z = 0; z < ndilutions; ++ z ) {
                    last_record_not_zero = std::all_of( matrix[l][z][ngroups - 1].begin(), matrix[l][z][ngroups - 1].end(), [](double val) {
                        return val > 0.;
                    });
                }
            }
