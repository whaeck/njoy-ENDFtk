...
makeMatrices(const std::vector< DataRecord > records, 
    ...
    for ( const auto& record : records) {
        if ( record.IG() == 0 ) {
            ...
            for ( unsigned int g_o = record.IG2LO() - 1; g_o < record.IG(); ++g_o ){ // g_o = outgoing_erg
                auto g_i = record.IG() - 1; // g_i = incident_erg
                for (unsigned int entry = 0; entry < record.NG2(); ++entry) {
                    for ( unsigned int l = 0; l < nmoments; ++l ) {
                        if ( flux[l].size() == 0 ) {
                        ...
                        for ( unsigned int z = 0; z < ndilutions; ++z ) {
                            ...
                            if (entry == 0) {
                                ...
                                matrix[l][z][g_i][g_o] = record.list()[entry * ndilutions * nmoments + z * nmoments + l];
                            }
                        }
                    }
                }
