...
class Test_ENDFtk_MF2_MT151_RMatrixLimited( unittest.TestCase ) :
    ...
    chunk = ( ' 0.000000+0 0.000000+0          0          3          2          02625 2151     \n'
    ...
    no_spin_groups = ( ' 0.000000+0 0.000000+0          0          3          0          02625 2151     \n'
                       ' 0.000000+0 0.000000+0          2          0         24          42625 2151     \n'
                       ' 0.000000+0 5.446635+1 0.000000+0 2.600000+1 1.000000+0 0.000000+02625 2151     \n'
                       ' 0.000000+0 0.000000+0 0.000000+0 1.020000+2 0.000000+0 0.000000+02625 2151     \n'
                       ' 1.000000+0 5.347624+1 0.000000+0 2.600000+1 5.000000-1 0.000000+02625 2151     \n'
    ...
    def test_component( self ) :
        ...
        def verify_chunk( self, chunk ) :
            ...
            self.assertEqual( False, chunk.KRL )
            self.assertEqual( False, chunk.non_relativistic_kinematics )
            self.assertEqual( 3, chunk.KRM )
            self.assertEqual( 3, chunk.formalism )

            self.assertAlmostEqual( 0., chunk.SPI )
            self.assertAlmostEqual( 0., chunk.spin )
            self.assertAlmostEqual( 5.437300e-1, chunk.AP )
            self.assertAlmostEqual( 5.437300e-1, chunk.scattering_radius )
        ...
        chunk = RMatrixLimited( ifg = False, krl = False, krm = 3,
                                pairs = pairs, groups = groups )

        verify_chunk( self, chunk )

        # the data is read from a string
        chunk = RMatrixLimited.from_string( self.chunk, 2625, 2, 151 )

        verify_chunk( self, chunk )

        # the data is copied
        copy = RMatrixLimited( chunk )
    ...
    def test_failures( self ) :
        ...
        with self.assertRaises( Exception ) :
            ...
            chunk = RMatrixLimited.from_string( self.no_spin_groups, 1025, 2, 151 )
